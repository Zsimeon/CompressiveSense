...
%   y = Phi * x                          % 观测信号 = 测量矩阵 * 信号
%   x = Psi * theta                      % 信号 = 稀疏矩阵 * 稀疏系数
%   y = Phi * Psi * theta                % 观测信号 = 测量矩阵 * 稀疏矩阵 * 稀疏系数     
%   令 A = Phi * Psi, 则y = A * theta    % 感知矩阵 = 测量矩阵 * 稀疏矩阵
%   现在已知y和A，求theta  
[y_rows,y_columns] = size(y);  %[y_行,y_列]
 if y_rows<y_columns 
     y = y';            %y should be a column vector  
 end
 [M,N] = size(A);       %传感矩阵A为M*N矩阵  size(A) 返回传感矩阵A的行数和列数
...
%      theta_ls = (At(:,1:ii)'*At(:,1:ii))^(-1)*At(:,1:ii)'*y;%最小二乘解 
     theta_ls = pinv(At(:,1:ii)'*At(:,1:ii))*At(:,1:ii)'*y;%最小二乘解 
